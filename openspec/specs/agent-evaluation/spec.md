...
## Purpose
How the conversation agent is measured before it is trusted: guardrail probes run one turn at a time, full flows run against scripted lead personas, deterministic assertions gate the LLM judge, and the whole suite runs on demand rather than in CI because it calls a real model (spec section 6).
## Requirements
