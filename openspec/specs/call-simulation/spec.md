# call-simulation Specification

## Purpose
Running the conversation agent against a stored lead in text and persisting the result as a real call attempt, so the whole pipeline - conversation, extraction, scoring, narrative, judge and state machine - is exercised without telephony. Admin-only, refused for an opted-out lead or an attempt already in flight, and labelled wherever attempts are shown.
## Requirements
