# conversation-script Specification

## Purpose
How the agent's system prompt is built: a fixed frame written in code carrying the AI disclosure, the tone rules, every spec section 6 guardrail and the call budget, with the active qualification criteria filling only the middle, in the lead's language and in the derived call order. Criterion text is data and can never countermand the frame.
## Requirements
