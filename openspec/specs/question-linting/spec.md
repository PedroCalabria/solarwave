# question-linting Specification

## Purpose
The advisory check that runs when an employee saves a criterion: a cheap model reads the question and warns about tone or about a conflict with a spec section 6 guardrail. It is advice, never authority - it cannot block a save, cannot delay one, and returns no warnings on any failure.
## Requirements
